...
impl Skill {
    pub fn from_json(js: &JsonSkill, lv: u8, owner: Option<&JsonCard>) -> Skill {
        ...
        if let Some(jc) = owner {
            if js.target.owner_school != 0 {
                target_mask |= school_mask(member_ofs(jc.member) / 3);
            }
...
pub fn mask_for(jc: &JsonCard) -> u64 {
    member_mask(jc.member) | role_mask(jc.role) | attribute_mask(jc.attribute)
}

fn process_effect(sd: &SkillData) -> SkillEff {
    use SkillEff::*;
    use ValueType::*;
    use SkillEffectType::*;
    let (effect_type, effect_value) = (sd.1, sd.2);
    match effect_type {
